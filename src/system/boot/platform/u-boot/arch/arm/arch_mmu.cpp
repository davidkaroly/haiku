...
fdt_map_memory_ranges(void* fdt, const char* path, bool physical = false)
{
	...
	int32 regAddressCells = 2;
	int32 regSizeCells = 2;
	...
	for (int32 i = 0; len; i++) {
		...
		if (physical) {
			...
			mmu_map_identity(base, base + size,
...
map_page(addr_t virtualAddress, addr_t physicalAddress, uint32 flags)
{
	...
	pageTable[pageTableIndex] = physicalAddress | flags | sSmallPageType;
...
init_page_directory(void* fdt)
{
	...
	mmu_map_identity((addr_t)&_start, (addr_t)&_end, ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | ARM_MMU_L2_FLAG_AP_KRW);

	// map our page directory region (TODO should not be identity mapped)
	mmu_map_identity((addr_t)sPageDirectory, sPageTableRegionEnd, ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | ARM_MMU_L2_FLAG_AP_KRW);

	// map our well known / static pages
	map_pages_loader();

	// map UART - identity mapping to be used by booyloader
	mmu_map_identity(gKernelArgs.arch_args.uart.regs.start,
		gKernelArgs.arch_args.uart.regs.start + gKernelArgs.arch_args.uart.regs.size,
		ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_AP_KRW | ARM_MMU_L2_FLAG_XN);

	// map UART to a high address for the kernel
	addr_t uart_addr = get_next_virtual_page(B_PAGE_SIZE);
	map_page(uart_addr, gKernelArgs.arch_args.uart.regs.start,
		ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_AP_KRW | ARM_MMU_L2_FLAG_XN);
	...
	/* initialize TTBCR to zero (LPAE off, use only TTBR0) */
...
{
	TRACE("mmu_map_physical_memory(phAddr=%lx, %lx)\n", physicalAddress, size);
...
mmu_allocate(void *virtualAddress, size_t size)
{
	...
	for (uint32 i = 0; i < size; i++) {
		map_page(get_next_virtual_page(B_PAGE_SIZE),
			get_next_physical_page(B_PAGE_SIZE), ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | ARM_MMU_L2_FLAG_AP_KRW);
	}
...
mmu_init_for_kernel(void)
{
	...
	virtPageDirectory = mmu_map_physical_memory((addr_t)sPageDirectory, 0x200000);
	...
	gKernelArgs.arch_args.next_pagetable = (uint32)(sNextPageTableAddress) - (uint32)sPageDirectory;
	gKernelArgs.arch_args.last_pagetable = (uint32)(sPageTableRegionEnd) - (uint32)sPageDirectory;
	...
	for (int i = 0; i < gKernelArgs.num_physical_memory_ranges; i++) {
	...
	for (int i = 0; i < gKernelArgs.num_physical_allocated_ranges; i++) {
		dprintf("    base 0x%08" B_PRIx64 ", length 0x%08" B_PRIx64 "\n",
	...
	for (int i = 0; i < gKernelArgs.num_virtual_allocated_ranges; i++) {
		dprintf("    base 0x%08" B_PRIx64 ", length 0x%08" B_PRIx64 "\n",
	...
	for (int i = 0; i < gKernelArgs.arch_args.num_virtual_ranges_to_keep; i++) {
