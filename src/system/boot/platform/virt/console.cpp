...
static void
refresh_framebuf(int32 x0, int32 y0, int32 w, int32 h)
{
	for (int32 y = y0; y < y0 + h; y++)
		for (int32 x = x0; x < x0 + w; x++) {
			...
			ramfb_clear(gFramebuf.Clip(charX, charY, gFixedFont.charWidth,
				gFixedFont.charHeight), bgColor);
			ramfb_blit_mask_rgb(gFramebuf, gFixedFont.ThisGlyph(cell % 0x100),
		...
		size_t bufferSize)
{
	...
	for (uint32 i = 0; i < bufferSize; i++) {
		if (string[0] == '\n') {
			...
			refresh_framebuf((sScreenOffset - 1) % sScreenWidth,
				(sScreenOffset - 1) / sScreenWidth, 1, 1);
		}

		if (sScreenOffset >= sScreenWidth * sScreenHeight) {
			scroll_up();
			refresh_framebuf(0, 0, sScreenWidth, sScreenHeight);
		}
...
static const struct console_keymap {
	int virtio_key;
	int console_key;
} kConsoleKeymap[] = {
	{   1, TEXT_CONSOLE_KEY_ESCAPE },
	{  14, TEXT_CONSOLE_KEY_BACKSPACE},
	{  28, TEXT_CONSOLE_KEY_RETURN },
	{  57, TEXT_CONSOLE_KEY_SPACE },
	{ 102, TEXT_CONSOLE_KEY_HOME },
	{ 103, TEXT_CONSOLE_KEY_UP },
	{ 104, TEXT_CONSOLE_KEY_PAGE_UP },
	{ 105, TEXT_CONSOLE_KEY_LEFT },
	{ 106, TEXT_CONSOLE_KEY_RIGHT },
	{ 107, TEXT_CONSOLE_KEY_END },
	{ 108, TEXT_CONSOLE_KEY_DOWN },
	{ 109, TEXT_CONSOLE_KEY_PAGE_DOWN },
};
...
console_wait_for_key(void)
{
	...
	for (size_t i = 0; i < B_COUNT_OF(kConsoleKeymap); i++) {
		if (kConsoleKeymap[i].virtio_key == key)
			return kConsoleKeymap[i].console_key;
	}
