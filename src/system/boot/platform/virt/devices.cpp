...
create_virtio_block_dev(int id)
{
	VirtioResources* devRes = find_virtio_dev(kVirtioDevBlock, id);
...
{
	for (int i = 0;; i++) {
		ObjectDeleter<VirtioBlockDevice> device(create_virtio_block_dev(i));
		if (!device.IsSet())
			break;
		TRACE("virtio_block[%d]\n", i);
