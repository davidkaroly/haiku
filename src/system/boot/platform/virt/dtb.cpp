...
dtb_handle_fdt(const void* fdt, int node)
{
	...
	if (device_type != NULL && strcmp(device_type, "memory") == 0) {
		gMemBase = fdt64_to_cpu(*((uint64*)fdt_getprop(fdt, node,
			"reg", NULL) + 0));
	...
	if (dtb_has_fdt_string(compatible, compatibleLen, "virtio,mmio")) {
		uint64* reg = (uint64*)fdt_getprop(fdt, node, "reg", NULL);
		virtio_register(
			fdt64_to_cpu(*(reg + 0)), fdt64_to_cpu(*(reg + 1)),
			dtb_get_interrupt(fdt, node));
	}

	if (dtb_has_fdt_string(compatible, compatibleLen, "qemu,fw-cfg-mmio")) {
		uint64* reg = (uint64*)fdt_getprop(fdt, node, "reg", NULL);
		gFwCfgRegs = (uint32_t)fdt64_to_cpu(*reg);
	}

	if (dtb_has_fdt_string(compatible, compatibleLen, "simple-framebuffer")) {
...
dtb_init(void* fdt)
{
	...
	dtb_handle_chosen_node(gFdt);

	int node = -1;
	int depth = -1;
	while ((node = fdt_next_node(gFdt, node, &depth)) >= 0 && depth >= 0) {
		dtb_handle_fdt(gFdt, node);
	}
}


void
dtb_copy_fdt(void **dest, void *src)
{
	uint32_t fdtSize = fdt_totalsize(src);

	// libfdt requires 8-byte alignment
	*dest = (void*)(addr_t)kernel_args_malloc(fdtSize, 8);

	if (*dest == NULL)
		panic("unable to malloc for FDT!\n");

	memcpy(*dest, src, fdtSize);
}
