	...
	addr_t kernelEntry, addr_t kernelArgs)
{
	...
	//addr_t satp = pageDirectory;
	SatpReg satp;
	satp.ppn = pageDirectory / B_PAGE_SIZE;
	satp.asid = 0;
	satp.mode = satpModeSv39;

	// Enter the kernel!
	dprintf("enter_kernel(satp: 0x%08" B_PRIxADDR ", kernelArgs: 0x%08" B_PRIxADDR ", "
		"kernelEntry: 0x%08" B_PRIxADDR ", sp: 0x%08" B_PRIx64 ")\n",
		satp.val, kernelArgs, kernelEntry,
		gKernelArgs.cpu_kstack[0].start + gKernelArgs.cpu_kstack[0].size);

	enter_kernel(satp.val, kernelArgs, kernelEntry,
		gKernelArgs.cpu_kstack[0].start + gKernelArgs.cpu_kstack[0].size);
}
