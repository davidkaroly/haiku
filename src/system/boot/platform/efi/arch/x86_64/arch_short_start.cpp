...
// From arch_short_mmu.cpp
extern void arch_mmu_init_short_gdt(gdt_idt_descr &bootGDTDescriptor);

extern void arch_short_mmu_post_efi_setup(size_t memoryMapSize,
	efi_memory_descriptor *memoryMap, size_t descriptorSize,
	uint32_t descriptorVersion);

extern uint32_t arch_short_mmu_generate_post_efi_page_tables(size_t memoryMapSize,
	efi_memory_descriptor *memoryMap, size_t descriptorSize,
	uint32_t descriptorVersion);


void
arch_short_start_kernel(addr_t kernelEntry)
{
	gdt_idt_descr bootGDTDescriptor;
	arch_mmu_init_short_gdt(bootGDTDescriptor);

	// Copy entry.S trampoline to lower 1M
	enter_kernel_t enter_kernel = (enter_kernel_t)0xa000;
	memcpy((void *)enter_kernel, (void *)arch_enter_kernel_short, B_PAGE_SIZE);
	...
	uint32_t pageDirectory = arch_short_mmu_generate_post_efi_page_tables(
	...
	arch_short_mmu_post_efi_setup(memoryMapSize, memoryMap,
	...
	dprintf("short_enter_kernel(pageDirectory: 0x%08" PRIx32 ", kernelArgs: 0x%08" B_PRIxADDR ", "
		"kernelEntry: 0x%08" B_PRIxADDR ", sp: 0x%08" B_PRIx64 ", bootGDTDescriptor: 0x%p)\n",
