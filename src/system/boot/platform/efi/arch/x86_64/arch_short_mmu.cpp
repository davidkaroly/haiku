...
dump_page_dir(void)
{
	...
	for (uint32_t i = 0; i < 1024; i++) {
		...
		if (directoryEntry != 0) {
			...
			uint32_t *pageTable = (uint32_t *)(addr_t)(directoryEntry & X86_PDE_ADDRESS_MASK);
...
void
arch_mmu_init_short_gdt(gdt_idt_descr &bootGDTDescriptor)
{
	...
	bootGDTDescriptor.base = (uint64_t)bootGDT;

	TRACE("gdt base = 0x%08" PRIx64 ", limit = %" PRIu16 "\n",
		bootGDTDescriptor.base, bootGDTDescriptor.limit);
}
...
map_page(addr_t virtAddr, phys_addr_t physAddr, uint32_t flags)
{
	...
	if (pageDirectoryEntry == 0) {
		...
		sPageDirectory[pageDirectoryIndex] = (uint32_t)(addr_t)pageTable | kDefaultPageTableFlags;
	} else {
		pageTable = (uint32_t *)(addr_t)(pageDirectoryEntry & X86_PDE_ADDRESS_MASK);
	}
...
map_range(addr_t virtAddr, phys_addr_t physAddr, size_t size, uint32_t flags)
{
	...
	if (virtAddr >= KERNEL_LOAD_BASE_32_BIT)
		ASSERT_ALWAYS(insert_virtual_allocated_range(virtAddr, size) >= B_OK);
}


void
arch_short_mmu_post_efi_setup(size_t memoryMapSize,
...
arch_mmu_allocate_page_directory(void)
{
	...
	sPageDirectory = (uint32_t *)ROUNDUP((addr_t)sPageDirectory, ALIGN_PAGEDIR);
	memset(sPageDirectory, 0, B_PAGE_SIZE);

	TRACE("sPageDirectory  = 0x%08" B_PRIxADDR "\n", (addr_t)sPageDirectory);
}


addr_t
arch_short_mmu_generate_post_efi_page_tables(size_t memoryMapSize,
	efi_memory_descriptor *memoryMap, size_t descriptorSize,
	uint32_t descriptorVersion)
{
	...
	sPageDirectory[1023] = (uint32_t)(addr_t)sPageDirectory | kDefaultPageFlags;

	addr_t virtPageDirectory;
	platform_bootloader_address_to_kernel_address((void*)sPageDirectory, &virtPageDirectory);

	gKernelArgs.arch_args.phys_pgdir = (uint32_t)(addr_t)sPageDirectory;
